...
func (r *CdrResolver) ProcessCdr(ctx context.Context, cdr db.Cdr) error {
	...
	if err != nil {
		dbUtil.LogOnError("LSP044", "Error retrieving session invoices", err)
	...
	if err != nil {
		dbUtil.LogOnError("LSP046", "Error retrieving session user", err)
	...
	case cdrTotalFiat > priceFiat:
		...
		sessionInvoice := r.SessionResolver.IssueSessionInvoice(ctx, user, sess, util.InvoiceParams{
